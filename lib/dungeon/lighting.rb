...
module Dungeon::Lighting
  ...
  def updatelight(type, radius, brightness, source_x, source_y, light_number=nil) # Update the lighting on a block
    if type == 'static' then
      @static_level = calculate_static_light( radius, brightness, @static_level, source_x, source_y, @x, @y )
    elsif type == 'dynamic' then
      @dynamic_array[light_number] = calculate_dynamic_light( radius, brightness, source_x, source_y, @x, @y )
      @dynamic_level = @dynamic_array.compact.max
    end
  end
  ...
  def updatecolor( preloaded=nil )
    if preloaded != nil then
      if @static_level >= @dynamic_level then
        ...
        biggest_dynamic_number = @dynamic_array.compact.max
  ...
  def get_preloaded_colors()
    result = []
    256.times do |i|
      result[i] = Gosu::Color.rgba(i, i, i, 250)
    end
    return result
  end
end
